...
# --- IMPORT CORE MODULE ---
Import-Module "$PSScriptRoot\CleanUpCore.psm1" -Force
    ...
    foreach($taskKey in $taskList.Keys){
        ...
        try{
            switch($taskKey){
                ...
                "BrowserCache"{
                    try {
                        $result = Clean-BrowserCache
                        $logBox.AppendText("[$((Get-Date).ToString('HH:mm:ss'))] [OK] Đã xóa cache trình duyệt ($($result.TotalFilesDeleted) files từ $($result.CleanedBrowsers.Count) browsers)`n")
                        if ($result.Errors.Count -gt 0) {
                            foreach ($err in $result.Errors) {
                                $logBox.AppendText("[$((Get-Date).ToString('HH:mm:ss'))] [WARN] $err`n")
                            }
                        }
                    } catch {
